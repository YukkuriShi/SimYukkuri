...
public class ToyLogic {
	...
	public static final boolean checkToy(Body b) {
		...
		if (found != null) {
			if (minDistance <= Translate.distance(0, 0, b.getStep(), b.getStep())) {
				if (found.getZ() != 0) {
					if (b.getFavItem(Body.FavItemType.BALL) == found && !found.isOwned(b)) {
						if (b.isRude()) {
				...
				if (!found.isOwned(b)) {
					found.setOwner(b);
					b.setFavItem(Body.FavItemType.BALL, found);
					if (!b.isTalking()) {
	...
	public static final boolean checkSui(Body b) {
		...
		boolean ret = false;
		Obj found = b.getFavItem(Body.FavItemType.SUI);
