...
public abstract class Body extends Obj implements java.io.Serializable {
	...
	protected static final float[] bodySize = {0.25f, 0.5f, 1.0f};
	...
	protected int VALUEPURCHASE[] = {2, 2, 2};   // These are set in src.yukkuri files for each yukkuri, this is just a placeholder.
	protected int VALUESALE[] = {1, 1, 1};
	...
	protected AgeState bodyAgeState;				// 霄ｫ菴灘ｹｴ鮨｢ BABY, CHILD, ADULT
	protected AgeState mindAgeState;				// 邊ｾ逾槫ｹｴ鮨｢ BABY, CHILD, ADULT
	protected int damage = 0;						// 闢�ｩ阪ム繝｡繝ｼ繧ｸ counter indicating damage
	protected int stress = 0;						// 闢�ｩ阪せ繝医Ξ繧ｹ
	protected int tang = 500;						// 闊後�閧･縺�
	...
	protected Attitude attitude = Attitude.AVERAGE;	// 諤ｧ譬ｼ counter indicating shithead/nicehead etc.
	protected Intelligence intelligence = Intelligence.AVERAGE;	// 遏･諤ｧ
	...
	protected boolean hasPants = false;				// 縺翫￥繧九∩譛臥┌ true if having pants
	...
	protected ArrayList<Attachment> attach = new ArrayList<Attachment>();
	protected Obj linkParent = null;
	// 遞ｮ譌丞挨縺ｮ迚ｹ谿願�蜉�
	protected boolean likeBitterFood = false;	// 闍ｦ縺�∴縺輔′螂ｽ縺阪°
	protected boolean likeHotFood = false;		// 霎帙＞縺医＆縺悟･ｽ縺阪°
	...
	protected int targetPosOfsX = 0;
	protected int targetPosOfsY = 0;
 ...
 */
	...
	// 繧､繝吶Φ繝�
	protected ArrayList<EventPacket> eventList = new ArrayList<EventPacket>(); // 縺薙�蛟倶ｽ薙↓蟇ｾ縺励※逋ｺ陦後＆繧後◆繧､繝吶Φ繝医�繝ｪ繧ｹ繝�
	...
	// 縺頑ｰ励↓蜈･繧�
	protected HashMap<FavItemType, Obj> favItem = new HashMap<Body.FavItemType, Obj>();
	...
	{
		if (hungry < HUNGRYLIMIT[bodyAgeState.ordinal()]/4) {
			return Hunger.NONE;
		}
		return Hunger.VERY;
	}
	...
	{
		if (damage > DAMAGELIMIT[bodyAgeState.ordinal()]) {
			toDead();
			return Damage.VERY;
		}
		if (damage >= DAMAGELIMIT[bodyAgeState.ordinal()]/2) {
			return Damage.VERY;
		}
		return Damage.NONE;
	}
	...
	private void checkDamage() {
		...
		else if (hungry >= HUNGRYLIMIT[bodyAgeState.ordinal()]) {
			damage += TICK;
		}
		if(criticalDamege == CriticalDamegeType.CUT) {
			damage += TICK*100;
		...
		if ( Terrarium.poisonSteam ){
			...
			if ( damageState == Damage.VERY ) {
				...
				setMessage(MessagePool.getMessage(this, MessagePool.Action.PoisonDamage), HOLDMESSAGE, false, true);
			}
		}
		if (damage < 0) {
			damage = 0;
	...
	private boolean checkShit() {
		...
		if(eating || peropero || sukkiri) return false;
		...
		if(criticalDamege == CriticalDamegeType.CUT) {
			if (shit > SHITLIMIT[bodyAgeState.ordinal()] - TICK*SHITSTAY*2) {
		...
		if(toShit && moveTarget instanceof Toilet) {
...
//				}
			...
			else if((attitude == Attitude.NICE || attitude == Attitude.VERY_NICE)
					|| (attitude == Attitude.AVERAGE && intelligence == Intelligence.WISE)) {
				// 諤ｧ譬ｼ縺悟埋濶ｯ縺区勸騾壹〒繧ら衍閭ｽ縺碁ｫ倥￠繧後�繝医う繝ｬ縺ｫ逹�￥縺ｾ縺ｧ150%縺ｾ縺ｧ謌第�縺ｧ縺阪ｋ
				if(shit < (SHITLIMIT[bodyAgeState.ordinal()] * 1.5f)) {
					shitting = false;
					cantMove = false;
					return cantMove;
				}
		...
		// 髯千阜繧定ｶ�∴縺溷�蜷医�繝√ぉ繝�け
		if ( shit > SHITLIMIT[bodyAgeState.ordinal()] ) {
	...
	private boolean checkChildbirth() {
		...
		if ( hasBabyOrStalk() || !hasBabyOrStalk() && birth) {
			...
			if (pregnantPeriod > PREGPERIOD) {
				...
				if ((hasPants || (lockmove && !shitting)) && babyTypes.size() > 0) {// 蜍輔￠縺ｪ縺�ｿｽ蜉�
					// 縺願�縺ｮ襍､繧�□縺代け繝ｪ繧｢
					babyTypes.clear();
					setDirty(true);
					if(lockmove && !hasPants){
						hasPants=true;
						setMessage(MessagePool.getMessage(this, MessagePool.Action.Breed2));
						hasPants=false;
					}else{
	...
	private void checkEmotion() {
		...
		if(criticalDamege == CriticalDamegeType.CUT) return;
		...
		if (noHungryPeriod > RELAXPERIOD && noDamagePeriod > RELAXPERIOD
			...
			if (!exciting && !relax) {
				...
				if (((bodyAgeState == AgeState.ADULT && !hasBabyOrStalk() ) || rapist) && r == 0) {
					exciting = true;
					excitingPeriod = 0;
					if(isRaper()) {
	...
	private void checkSick() {
		if ( ( ( dirty && damage > ( DAMAGELIMIT[bodyAgeState.ordinal()])/2 ) || ( Terrarium.humid && damage > 0 )) && !Terrarium.antifungalSteam ) {
			if (Terrarium.humid){
				dirtyPeriod += TICK*4;
			}else{
				dirtyPeriod += TICK;
	...
	//		}
			...
			if (burnTimer == BURNLIMIT[bodyAgeState.ordinal()] || burnTimer > BURNLIMIT[bodyAgeState.ordinal()])
			{
	...
	private void checkMessage() {
		...
		if (dead) {
			...
			} else if (!flyingType && getZ() > 15 && panicType == null && !lockmove && criticalDamege != CriticalDamegeType.CUT) {
				setMessage(MessagePool.getMessage(this, MessagePool.Action.Flying), HOLDMESSAGE, false, true);
				addStress(-10);
	...
	public void upDate(){
		...
		if ( getStalks().size() > 0 ) {
			...
			int centerH = (imgH[bodyAgeState.ordinal()] + getExpandSize() + getExternalForceW());
			int sX;
			int ofsX;
			int k = 0;
			for ( Stalk stalk: getStalks()) {
				if ( stalk != null ) {
					sX = stalk.getPivotX() + stalkOfsX[k] - (int)((3 - bodyAgeState.ordinal()) * 8.75f);
					if (direction == Body.RIGHT) {
					...
					stalk.setY( getY() );
					stalk.setZ( getZ() + (int)(centerH * 0.09f)  + stalkOfsY[k] );
					stalk.upDate();
				}
				k = (k + 1) & 7;
			}
	...
	private void moveBody(boolean dontMove) {
		...
		if (vz != 0 || (!canflyCheck() && z != 0)) {
			...
			if (z <= 0) {
				...
				int jumpLevel[] = {2, 2, 1};
				int damageCut = 1;
				if (falldownDamage >= 8/jumpLevel[bodyAgeState.ordinal()]) {
		...
		// moving
		int step = STEP[bodyAgeState.ordinal()];
		...
		if (destX >= 0) {
			...
			if (countX++ >= sameDest * STEP[bodyAgeState.ordinal()]) {
				countX = 0;
		...
		if (destY >= 0) {
			...
			if (countY++ >= sameDest * STEP[bodyAgeState.ordinal()]) {
				countY = 0;
	...
	public void setMessage(String message) {
		setMessage(message, WindowType.NORMAL, HOLDMESSAGE, true, false);
	}
	// 譎る俣謖�ｮ壹Γ繝�そ繝ｼ繧ｸ陦ｨ遉ｺ
	...
	public void setMessage(String message, boolean interrupt) {
		setMessage(message, WindowType.NORMAL, HOLDMESSAGE, interrupt, false);
	}
	// 蜈ｨ謖�ｮ壹Γ繝�そ繝ｼ繧ｸ陦ｨ遉ｺ
...
//			peropero = false;
			...
			nobinobi = false;
			pikopiko = piko;
			messageLineColor = windowColor[type.ordinal()][0];
			messageBoxColor = windowColor[type.ordinal()][1];
			messageTextColor = windowColor[type.ordinal()][2];
			messageWindowLine = windowLine[type.ordinal()];
			messageTextSize = 12;
		}
	}

	protected void setNegiMessage(String message, boolean piko) {
		setNegiMessage(message, HOLDMESSAGE, piko);
	}
	...
	protected void setNegiMessage(String message, int count, boolean piko) {
		...
		sukkiri = false;
		nobinobi = false;
		messageLineColor = negiWindowColor[0];
		messageBoxColor = negiWindowColor[1];
		messageTextColor = negiWindowColor[2];
		messageWindowLine = windowLine[0];
		messageTextSize = 120;
	}
	...
	public Dna getDna() {
		Dna ret = new Dna(getType(), attitude, intelligence, false);
	...
	public int getDamageLimit() {
		return DAMAGELIMIT[bodyAgeState.ordinal()];
	}
	...
	public int getHungryLimit() {
		return HUNGRYLIMIT[bodyAgeState.ordinal()];
	}
	...
	public int getShitLimit() {
		return SHITLIMIT[bodyAgeState.ordinal()];
	}
	...
	public int getStressLimit() {
		return STRESSLIMIT[bodyAgeState.ordinal()];
	}
	...
	public FootBake getFootBakeLevel() {
		...
		if(footBakePeriod > DAMAGELIMIT[bodyAgeState.ordinal()]) {
			ret = FootBake.CRITICAL;
		} else if(footBakePeriod > (DAMAGELIMIT[bodyAgeState.ordinal()] >> 1)) {
			ret = FootBake.MIDIUM;
		}
		return ret;
	}
	...
	public int getEatAmount() {
		return EATAMOUNT[bodyAgeState.ordinal()];
	}

	public int getStep() {
		return STEP[bodyAgeState.ordinal()];
	}
	...
	public void getExpandShape(Rectangle r) {
		...
		r.width = imgW[bodyAgeState.ordinal()] + getExpandSize() + forceW;
		r.height = imgH[bodyAgeState.ordinal()] + getExpandSize() + forceH;
		r.x = r.width >> 1;
		r.y = r.height - 1;
	}
	
	public int getCollisionX() {
		return (imgW[bodyAgeState.ordinal()] + getExpandSize()) >> 1;
	}

	// 螯雁ｨ��縺�ｓ縺�ｓ縺ｫ繧医ｋ菴灘梛縺ｮ縺ｵ縺上ｉ縺ｿ蜿門ｾ�
	private int getExpandSize() {
		return (20 - 20/(getBabyTypes().size() + 1 )) + getBabyTypes().size() * 2 + ( ( shit * 4 / 5 ) /SHITLIMIT[bodyAgeState.ordinal()] ) * 5;
	}
	...
	public int getSize() {
		return imgW[bodyAgeState.ordinal()] + getExpandSize();
	}
	
	public int getOriginSize() {
		return imgW[bodyAgeState.ordinal()];
	}
	
	public int getStrength() {
		return STRENGTH[bodyAgeState.ordinal()];
	}
	...
	public Image getShadowImage() {
		return shadowImages[bodyAgeState.ordinal()];		
	}

	public int getShadowH() {
		return shadowImgH[bodyAgeState.ordinal()];
	}
	...
	public int getH() {
		return imgH[bodyAgeState.ordinal()];
	}

	public int getPivotX() {
		return pivX[bodyAgeState.ordinal()];
	}

	public int getPivotY() {
		return pivY[bodyAgeState.ordinal()];
	}
	
	public void getBoundaryShape(Rectangle r) {
		r.x = pivX[bodyAgeState.ordinal()];
		r.y = pivY[bodyAgeState.ordinal()];
		r.width = imgW[bodyAgeState.ordinal()];
		r.height = imgH[bodyAgeState.ordinal()];
	}

	public int getBraidW() {
		return braidImgW[bodyAgeState.ordinal()];
	}

	public int getBraidH() {
		return braidImgH[bodyAgeState.ordinal()];
	}
	...
	public int getWeight() {
		return (WEIGHT[bodyAgeState.ordinal()] + (babyTypes.size() + stalkBabyTypes.size()) * 50);
	}
	...
	public static void loadShadowImages (ClassLoader loader, ImageObserver io) throws IOException {
		...
		shadowImages[adultIndex] = ImageIO.read(loader.getResourceAsStream(path+"shadow.png"));

		sx = (int)((float)shadowImages[adultIndex].getWidth(io) * bodySize[1]);
		sy = (int)((float)shadowImages[adultIndex].getHeight(io) * bodySize[1]);
		shadowImages[childIndex] = shadowImages[adultIndex].getScaledInstance(sx, sy, Image.SCALE_AREA_AVERAGING);
		sx = (int)((float)shadowImages[adultIndex].getWidth(io) * bodySize[0]);
		sy = (int)((float)shadowImages[adultIndex].getHeight(io) * bodySize[0]);
		shadowImages[babyIndex] = shadowImages[adultIndex].getScaledInstance(sx, sy, Image.SCALE_AREA_AVERAGING);
	...
	public boolean isSoHungry() {
		return (!dead && (hungry >= HUNGRYLIMIT[bodyAgeState.ordinal()] * 0.8f));
	}

	public boolean isFull() {
		return (!dead && (hungry <= HUNGRYLIMIT[bodyAgeState.ordinal()] / 20));
	}

	public boolean isTooHungry() {
		return (!dead && hungry >= HUNGRYLIMIT[bodyAgeState.ordinal()] && checkDamageState() == Damage.VERY);
	}
	...
	public boolean isAdult() {
		return (bodyAgeState == AgeState.ADULT);
	}
	
	public boolean isChild() {
		return (bodyAgeState == AgeState.CHILD);
	}
	
	public boolean isBaby() {
		return (bodyAgeState == AgeState.BABY);
	}
	...
	public boolean wantToShit() {
		...
		if ((SHITLIMIT[bodyAgeState.ordinal()] - shit) < (DIAGONAL * step + adjust)) {
			return true;
		}
		return false;
	}
	...
	public boolean nearToBirth() {
		...
		int diagonal = DIAGONAL * step + adjust; 
...
//		p.clearActions();
	...
	public void forceToSleep() {
		...
		if(panicType == PanicType.BURN || criticalDamege != null) {
			return;
		}
	...
	public void strikeByHammer() {
		...
		if (dead) {
			...
			if (bodyAgeState != AgeState.ADULT) {
				crashed = true;
			}
		}
	}
...
//System.out.println(vecX+","+vecY+":"+kick);
	...
	public void bodyCut() {
		clearActions();
		criticalDamege = CriticalDamegeType.CUT;
		for(int i = 0; i < 5; i++) {
	...
	public void kick() {
		int blowLevel[] = {-4, -3, -2};
		kick(0, blowLevel[bodyAgeState.ordinal()]*2, blowLevel[bodyAgeState.ordinal()]);
		strikeByPunish();
	}
	...
	*/
	...
	public void lockSetZ(int force) {
		...
		if(extForce < 0) {
			// 縺､縺ｶ繧�
			if(extForce < extForcePushLimit[bodyAgeState.ordinal()]) {
				// 蝨ｧ豁ｻ
				lockmove = false;
				extForce = 0;
				setZ(0);
				bodyBurst();
			} else if(extForce < (extForcePushLimit[bodyAgeState.ordinal()] >> 1)) {
				// 髯千阜
				if(rnd.nextInt(10) == 0){
					setHappiness(Happiness.VERY_SAD);
					setMessage(MessagePool.getMessage(this, MessagePool.Action.Press2), HOLDMESSAGE, true, true);
					addStress(10);
				}
				if(rnd.nextInt(80) == 0){
					int ofsX = Translate.invertX(getCollisionX()>>1, y);
					if(getDirection() == Body.Direction.LEFT) ofsX = -ofsX;
					SimYukkuri.mypane.terrarium.addVomit(getX() + ofsX, getY() + 2, getZ(), bodyAgeState, shitType);
			...
			// 縺ｲ縺｣縺ｱ繧�
			if(extForce > extForcePullLimit[bodyAgeState.ordinal()]) {
				// 縺｡縺弱ｌ
				extForce = 0;
				lockmove = false;
				bodyCut();
			} else if(extForce > extForcePullLimit[bodyAgeState.ordinal()] >> 1){
				// 髯千阜
				if(rnd.nextInt(10) == 0) {
					setHappiness(Happiness.VERY_SAD);
					setMessage(MessagePool.getMessage(this, MessagePool.Action.Pull2), HOLDMESSAGE, true, true);
					addStress(10);
				}
			} else {
				if(rnd.nextInt(10) == 0){
...
//		}
	...
	@Override
	public int getValue() {
		return value[bodyAgeState.ordinal()];
	}
	...
	public int getBodyBaseImage(Image[] layer, int[] mode) {
		...
		if ( isShitting() || isBirth() && babyTypes.size() > 0) {
			// 謗呈ｳ��蜃ｺ逕｣譎�
			if(analClose) {
				layer[idx] = getImage(Body.FRONT_SHIT_SEALED,Body.LEFT);
			} else {
				layer[idx] = getImage(Body.FRONT_SHIT,Body.LEFT);
			}
			idx++;

			if(hasPants()) {
				layer[idx] = getImage(Body.FRONT_PANTS,Body.LEFT);
				idx++;
			}
			if(okazariType == OkazariType.DEFAULT) {
				layer[idx] = getImage(Body.ROLL_ACCESSORY,Body.LEFT);
				idx++;
			}
		}
			...
			if(analClose) {
				layer[idx] = getImage(Body.ROLL_SHIT_SEALED, dir);
			} else {
				layer[idx] = getImage(Body.ROLL_SHIT, dir);
			}
			idx++;

			if(hasPants()) {
				layer[idx] = getImage(Body.ROLL_PANTS,dir);
				idx++;
			}
			if(okazariType == OkazariType.DEFAULT) {
				layer[idx] = getImage(Body.ROLL_ACCESSORY,Body.LEFT);
			...
			// 貎ｰ繧後◆豁ｻ菴�
			if(isBurned()) {
				layer[idx] = getImage(Body.BURNED2,Body.LEFT);
				idx++;
			} else {
				if(okazariType == OkazariType.DEFAULT) {
					layer[idx] = getImage(Body.CRUSHED,Body.LEFT);
				} else {
					layer[idx] = getImage(Body.CRUSHED2,Body.LEFT);
				}
				idx++;
			}
		} else if(isBurned() && isDead()) {
			// 辟ｼ豁ｻ菴�
			layer[idx] = getImage(Body.BURNED,Body.LEFT);
			idx++;
		} else {
			// 騾壼ｸｸ譎�
			layer[idx] = getImage(Body.BODY, direction);
			idx++;
			if(okazariType == OkazariType.DEFAULT) {
				layer[idx] = getImage(Body.ACCESSORY, direction);
				idx++;
	...
	public int getFaceImage(Image[] layer, int[] mode) {
		...
		if(isDead()) {
			layer[idx] = getImage(Body.DEAD,direction);
			idx++;
		} else if(criticalDamege == CriticalDamegeType.CUT) {
			layer[idx] = getImage(Body.PAIN,direction);
			idx++;
		} else if(isExciting()) {
			layer[idx] = getImage(Body.EXCITING,direction);
			idx++;
		} else if(isSleeping()) {
			layer[idx] = getImage(Body.SLEEPING,direction);
			idx++;
		} else if(isPeroPero() || isEating()) {
			if (isStrike() || isVerySad() || isFeelHardPain()) {
				layer[idx] = getImage(Body.CRYING,direction);
			} else if(isSad() || isEatingShit() || isFeelPain()) {
				layer[idx] = getImage(Body.TIRED, direction);
			} else {
				layer[idx] = getImage(Body.SMILE, direction);
			}
			idx++;
		} else if(isSukkiri()) {
			layer[idx] = getImage(Body.REFRESHED,direction);
			idx++;
			...
			if(panicType != null) {
				...
				layer[idx] = getImage(Body.CHEER,direction);
			} else if(isStrike() || isVerySad()) {
				layer[idx] = getImage(Body.CRYING,direction);
			} else if(isAngry()) {
				layer[idx] = getImage(Body.PUFF,direction);
			} else if(isSad() || isOld()) {
				layer[idx] = getImage(Body.TIRED,direction);
			} else if(isHappy() || isNobinobi()) {
				layer[idx] = getImage(Body.SMILE, direction);
			} else if(isTalking() && isRude()) {
				layer[idx] = getImage(Body.RUDE,direction);
			} else if(isTalking() && !isRude()) {
				layer[idx] = getImage(Body.CHEER,direction);
			} else {
				layer[idx] = getImage(Body.NORMAL,direction);
			}
			idx++;
		}
		return idx;
	...
	public int getEffectImage(Image[] layer, int[] mode) {
		...
		if(burned && cooked && burnedDontMove) {
			layer[idx] = getImage(Body.BURN, direction);
			idx++;
		} else if(burned && !burnedDontMove && isOnFire()) {
			layer[idx] = getImage(Body.BURN01, direction);
			idx++;
		}
		...
		if(f == FootBake.MIDIUM ) {
			layer[idx] = getImage(Body.FOOT_BAKE0, direction);
			idx++;
		} else if(f == FootBake.CRITICAL) {
			layer[idx] = getImage(Body.FOOT_BAKE1, direction);
			idx++;
		}
		// 繝�Γ繝ｼ繧ｸ
		if(damage > DAMAGELIMIT[bodyAgeState.ordinal()] * 0.75f) {
			layer[idx] = getImage(Body.DAMAGED2, direction);
			idx++;
		} else if(damage > DAMAGELIMIT[bodyAgeState.ordinal()] * 0.5f) {
			layer[idx] = getImage(Body.DAMAGED1, direction);
			idx++;
		} else if(damage > DAMAGELIMIT[bodyAgeState.ordinal()] * 0.25f) {
			layer[idx] = getImage(Body.DAMAGED0, direction);
			idx++;
		} else if(isOld()) {
			layer[idx] = getImage(Body.DAMAGED2, direction);
			idx++;
		...
		// 縺翫￥繧九∩
		if(hasPants()) {
			layer[idx] = getImage(Body.PANTS, direction);
			idx++;
		}
		if(isPinned()) {
			layer[idx] = getImage(Body.BODYPINNED, direction);
			idx++;
		}
		// 雜ｳ豎壹ｌ
		if(isDirty()) {
			layer[idx] = getImage(Body.STAIN, direction);
			idx++;
		}
		// 縺九�
		if(sickPeriod > (INCUBATIONPERIOD << 6)) {
			layer[idx] = getImage(Body.SICK2, direction);
			idx++;
		} else if(sickPeriod > (INCUBATIONPERIOD << 3)) {
			layer[idx] = getImage(Body.SICK1, direction);
			idx++;
		} else if(sickPeriod > INCUBATIONPERIOD) {
			layer[idx] = getImage(Body.SICK0, direction);
			idx++;
		}
		// 豼｡繧�
		if(isWet()) {
			layer[idx] = getImage(Body.WET, direction);
			idx++;
		}
		return idx;
	}
	...
	public int getBraidImage(Image[] layer, int[] mode) {
		...
		if(hasBraidCheck()) {
			...
			if(canflyCheck()) {
				// 鬟幄｡檎憾諷�
				layer[idx] = getImage((int)(Body.BRAID_MV0 + ((age % 6) >> 1)), direction);
				idx++;
			} else {
				if(pikopiko) {
					layer[idx] = getImage((int)(Body.BRAID_MV0 + ((age % 6) >> 1)), direction);
					...
					layer[idx] = getImage(Body.BRAID, direction);
					idx++;
				}
			}
		} else {
			// 遐ｴ螢顔憾諷�
			layer[idx] = getImage(Body.BRAID_CUT, direction);
			idx++;
		}
		return idx;
	}
	...
	private boolean canEventResponse() {
		...
		if(criticalDamege == CriticalDamegeType.CUT) return false;
...
*/	
	...
	public Obj.Event clockTick() {
		...
		if(Terrarium.getInterval() == 0) {
			if(Terrarium.ageBoostSteam && bodyAgeState != AgeState.ADULT) addAge(10000);
		...
		// 蟷ｴ鮨｢繝√ぉ繝�け
		AgeState curAge = bodyAgeState;
		FootBake foot = getFootBakeLevel();
		bodyAgeState = checkBodyAgeState();
		if(!curAge.equals(bodyAgeState)) {
			// 蜉�ｽ｢
			initAmount(bodyAgeState);
			// DamageLimit繧呈ｵ∫畑縺励※繧九ヱ繝ｩ繝｡繝ｼ繧ｿ縺ｯ迥ｶ諷九ｒ邯ｭ謖√☆繧九◆繧√％縺薙〒蜀崎ｨ育ｮ�
			switch(foot) {
				case MIDIUM:
					footBakePeriod = (DAMAGELIMIT[bodyAgeState.ordinal()] >> 1) + 1;
					break;
				case CRITICAL:
					footBakePeriod = DAMAGELIMIT[bodyAgeState.ordinal()] + 1;
					break;
				default:
					break;
			}
		}
		mindAgeState = checkMindAgeState();
		...
		if(panicType != null) {
			...
			if (lockmove || criticalDamege != null || (getFootBakeLevel() == FootBake.CRITICAL && !canflyCheck())) {
				dontMove = true;
			}
			setHappiness(Happiness.VERY_SAD);
			checkDamage();
	...
	if (!isBusy()){
		...
		int oldShit = shit;
		// check shit
		if (checkShit() || criticalDamege != null || (getFootBakeLevel() == FootBake.CRITICAL && !canflyCheck())) {
			dontMove = true;
		}
		if ( shitting == false && oldShit != 0 && shit == 0) {
			if (!hasPants()) {
	...
	public Body(int initX, int initY, int initZ, AgeState initAgeState, Body mama, Body papa ) {    /// Create new yukkuri stats.  Needs rewrite
		...
		if(papa != null && papa.getFavItem(Body.FavItemType.BED) != null) {
			setFavItem(Body.FavItemType.BED, papa.getFavItem(Body.FavItemType.BED));
		} else if(mama != null && mama.getFavItem(Body.FavItemType.BED) != null) {
			setFavItem(Body.FavItemType.BED, mama.getFavItem(Body.FavItemType.BED));
		}
		...
		age += rnd.nextInt(100);
		bodyAgeState = checkBodyAgeState();
		mindAgeState = bodyAgeState;
		initAmount(initAgeState);
		wakeUpTime = age;
		shit = rnd.nextInt(SHITLIMIT[bodyAgeState.ordinal()]);
		if (bodyAgeState == AgeState.BABY) {
			if (mama != null) {
				if (mama.isDamaged()) {
