...
*/
public class StealOkazariEvent extends EventPacket implements java.io.Serializable {

	private static final long serialVersionUID = 1L;
	Random rnd = new Random();

	public StealOkazariEvent(Body f, Body t, Obj tgt, int cnt) {
		super(f, t, tgt, cnt);
	}
	...
	public boolean checkEventResponse(Body b) {
		...
		// 蝟�憶縺ｯ蜿ょ刈縺励↑縺�
		if(b.isSmart() || !b.isRude()) return false;
		// 閾ｪ蛻�′雉｢縺��蜷医�縺翫°縺悶ｊ縺後↑縺上※繧ょｮｶ譌上ｒ隱崎ｭ倥＠縺ｦ蜿ょ刈縺励↑縺�
		if(b.getIntelligence() == Intelligence.AVERAGE) {
		...
		if(ret) {
			if(getFrom() != b) {
				String stealOkazari = to.getNameE() + "'s mister accessory is mine easy!";
				b.setWorldEventResMessage(stealOkazari, ConstantValues.HOLDMESSAGE, true, false);
	...
	public UpdateState update(Body b) {
		...
		if(to.isRemoved() || b.hasOkazari() || !to.hasOkazari()) return UpdateState.ABORT;
		if(to.getType() != b.getType() && (b.getIntelligence() == Intelligence.WISE || b.getIntelligence() == Intelligence.AVERAGE)) return UpdateState.ABORT;
	...
	public boolean execute(Body b) {
		...
		if(!to.isDead() && to.hasOkazari() && !to.isRemoved() && to.getZ() < 5) {
			String stealOkazari = to.getNameE() + "'s mister accessory is mine easy!";
			b.setWorldEventResMessage(stealOkazari, ConstantValues.HOLDMESSAGE, true, false);
